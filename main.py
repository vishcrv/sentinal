...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        # need to add a parsing logic based on the LLM output structure

        is_calendar_request = calendar_integration.detect_calendar_intent(
            request.message
        )

        if is_calendar_request:
            # Handle calendar scheduling
            calendar_result = await calendar_integration.schedule_from_message(
                message=request.message, user_id=request.user_id
            )

            response_text = calendar_result.get("message")
            calendar_event_created = calendar_result.get("success", False)

            # Get mood-based suggestions
            current_mood = user_data.get("current_mood", "calm")
            suggestions = await calendar_integration.suggest_wellness_activities(
                current_mood
            )
            suggestion_texts = [
                f"{s['title']} ({s['duration']} min)" for s in suggestions[:3]
            ]
...
async def schedule_wellness_activity(request: dict):
    """
    Schedule wellness activity from natural language.
    Same pattern as /api/spotify/recommend endpoint.
    """
    try:
        ...
        result = await calendar_integration.schedule_from_message(
...
async def suggest_calendar_activities(request: dict):
    """
    Get wellness activity suggestions based on mood.
    Mirrors /api/spotify/recommend pattern.
    """
    try:
        mood = request.get("mood", "calm")

        suggestions = await calendar_integration.suggest_wellness_activities(mood)

        return {"mood": mood, "activities": suggestions}
...
@app.get("/api/calendar/upcoming/{user_id}")
async def get_upcoming_wellness_events(user_id: str, days: int = 7):
    """
    ...
    try:
        events = await calendar_integration.get_upcoming_events(
            days_ahead=days, max_results=10
        )
