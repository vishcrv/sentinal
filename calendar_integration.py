...
async def create_wellness_event(
    ...
    try:
        ...
        # Try to extract Google event ID from result
        google_event_id = None
        try:
            import json
            result_data = json.loads(result) if isinstance(result, str) else result
            # The create_event function doesn't return event ID in the JSON
            # We'll need to get it from the service directly or store it separately
            # For now, we'll leave it as None and update it later if needed
        except:
            pass
