...
from chatbot_engine import analyze_mood_with_llm
...
def log_mood_entry(
    ...
    if entry_date:
        try:
            ...
            entry_timestamp = date_obj.replace(
    ...
    mod = analyze_mood_with_llm(notes)
    # Save to database
    db.save_mood_entry(
        user_id=user_id,
        entry_id=entry_id,
    ...
    print(
        f"📊 Mood logged: {mood} ({intensity}/10) for user {user_id} on {entry_timestamp.strftime('%Y-%m-%d')}"
    )
    return entry_id
...
def get_mood_insights(user_id: str) -> Dict:
    ...
    insights = {
        ...
        "most_common_mood": {"mood": most_common_mood[0], "count": most_common_mood[1]},
...
def generate_insight_message(insights: Dict) -> str:
    ...
    if count < 5:
        messages.append(
            f"You've logged {count} moods so far. Keep going to see better insights!"
        )
    else:
    ...
    if trend == "improving":
        ...
        messages.append(
            "I notice your mood has been lower recently. Want to talk about it? 💙"
        )
    elif trend == "stable":
    ...
    if avg_intensity >= 7:
        ...
        messages.append(
            "It's been a tough time. Remember, it's ok to reach out for support."
        )
